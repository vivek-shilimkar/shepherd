...
func CreateAlibabaCloudCredentials(client *rancher.Client, credentials cloudcredentials.CloudCredential) (*v1.SteveAPIObject, error) {
	...
	spec := corev1.Secret{
		...
		Data: map[string][]byte{
			"alibabacredentialConfig-accessKeyId":     []byte(credentials.AlibabaCredentialConfig.AccessKey),
			"alibabacredentialConfig-accessKeySecret": []byte(credentials.AlibabaCredentialConfig.SecretKey),
		},
